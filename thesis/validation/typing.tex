...
\begin{itemize}
    ...
    \item \textbf{Ins} specializes some polymorphic type (in regard to the type system implementation) to a more specific polymorphic type.
...
\end{minipage}
    ...
    If an efficient representation of \texttt{dup} was implemented such that the left and right side were shared such that $\mu \mapsto ((x \rightarrow x \rightarrow \tau) \rightarrow \tau)$, the number introduced type variables would be $\Omega(n)$.
...
\autoref{fig:expplot} shows the relationship between the program typed in $L$, the theoretical time ($2^n$).
\begin{figure}[ht]
    \centering
\begin{tikzpicture}
    \begin{axis}[
