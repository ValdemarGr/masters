...
\begin{lstlisting}[float,language=ML,caption={Head implementation},label={lst:headimpl},mathescape=true]
...
\begin{lstlisting}[float,language=ML,caption={First function},label={lst:fstimplbad},mathescape=true]
fun fst a b: $\forall$A.$\forall$B.A $\rightarrow$ B $\rightarrow$ A = a;
\end{lstlisting}
\begin{lstlisting}[float,language=ML,caption={First function in lambda calculus},label={lst:fstimpl},mathescape=true]
...
\begin{lstlisting}[float,language=ML,caption={Implementation of type state},label={lst:theoremsforfree},mathescape=true]
$\lambda$x.
    let z = ($\lambda$y.x) in
    $\dots$
\end{lstlisting}
...
\begin{lstlisting}[float,language=ML,caption={Identity function in $L$},label={lst:idfun}]
fun id x = x;
id 4;
\end{lstlisting}
\begin{lstlisting}[float,language=ML,caption={Identity function in lambda calculus with let},label={lst:idfunlam},mathescape=true]
...
\begin{lstlisting}[float,language=ML,caption={Identity function in lambda calculus by abstraction},label={lst:idfunlamabs},mathescape=true]
...
\begin{lstlisting}[float,language=ML,caption={Identity function with two applications},label={lst:idfunlam2},mathescape=true]
let id = ($\lambda$x.x) in
(id 4, id id)
\end{lstlisting}
\begin{lstlisting}[float,language=ML,caption={Identity function with two applications as abstraction},label={lst:idfunlamabs2},mathescape=true]
...
\begin{lstlisting}[float,language=ML,caption={Polymorphic id},label={lst:polyid},mathescape=true]
fun id x = x;
fun ap x f = f x;
...
\begin{lstlisting}[float,language=ML,caption={Recursive function},label={lst:recplus1},mathescape=true]
fun f x = (f x) + 1;
\end{lstlisting}
...
\begin{lstlisting}[float,language=ML,caption={ADT implementation of decision},label={lst:adtdecision},mathescape=true]
type Boolean = 
    | BFalse
    | BTrue
;
...
\begin{lstlisting}[float,language=ML,caption={Nested pair},label={lst:pairproof},mathescape=true]
fun dup a f = f a a;
...
\begin{lstlisting}[float,language=ML,caption={Nested pairs with different type variables},label={lst:pairproofexp},mathescape=true]
...
\begin{lstlisting}[float,language=ML,caption={Program that requires parametric polymorphism},label={lst:rankn},mathescape=true]
fun f makeNum a =
...
The relaxation of type constructions allow various types to be generalized such as $a \rightarrow b \rightarrow M a b$ which could also have the signature of $a \rightarrow b \rightarrow M b a$ which kinds abstract over generalizing $M$ to $* \rightarrow * \rightarrow *$.
...
\begin{remark}
    ...
    Just as some complications in type systems are resolved with weakening the type system or enriching the syntax, kinds can have been abstracted away into types~\cite{weirich2013system}.
\end{remark}
