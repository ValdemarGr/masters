...
fun id x = x;
fun doubleid x = id (id x);
\end{lstlisting}
    After inferring \texttt{id} the environment will contain id $\Gamma = \{ (\texttt{id}, \forall a . a \rightarrow a) \}$.
    Typing the function \texttt{doubleid} without the use of \textit{inst}, start with the introduced parameter \texttt{x} and then the innermost expression \texttt{id x}. 
\begin{align}
    &\textit{bound}(\tau) = \textit{free}(\tau) - \textit{free}(\Gamma) = \{ \tau \} \tag{\textbf{Abs} intro \texttt{x} : $\tau$ - \theequation}\\
    &\Gamma = \{ (\texttt{id}, \forall a . a \rightarrow a), (\texttt{x}, \forall \tau . \tau) \}\\
    &\textit{unify}(a \rightarrow a, \tau \rightarrow \mu) = \textit{unify}(\{ a \mapsto \tau \}a, \{ a \mapsto \tau \}\mu) \cdot \{ a \mapsto \tau \}\\
    &= \{ \tau \mapsto \mu \} \cdot \{ a \mapsto \tau \}\\
    &= \{ \tau \mapsto \mu, a \mapsto \mu \}
    \label{eq:unifyres}
\end{align}
    This example might not look compromising but a minor change such that the inner expression becomes \texttt{id (x + 1)} maps \texttt{x} to \texttt{Int} thus constrains \texttt{id} to only work on the type \texttt{Int} which is in violation with the outer application of \texttt{id}.
    The apparent problem is that in \autoref{eq:unifyres} \texttt{id} is generalized within its domain but specialized within \texttt{doubleid}.
    By instatiating quantified types when they are needed cases such as this can be avoided.
    \begin{align}
        &\textit{unify}(\textit{inst}(\forall a . a \rightarrow a), \textit{inst}(\forall \tau . \tau \rightarrow \mu))\\
        & = \textit{unify}(\gamma \rightarrow \gamma, \varphi \rightarrow \mu)\\
