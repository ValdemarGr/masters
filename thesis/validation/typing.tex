...
The spellchecking equivalent for computer programs could be type checking; the problem of validating a programmer's intuition of a program's intent.
...
There are two variants of types in Hindley-Milner, the \textit{monotype} and the \textit{polytype}.
...
The types $\tau_1 \dots \tau_n$ are the type variables required to construct some atomic type.
...
\begin{figure}[ht]
    \begin{mdframed}[style=bigbox]
        ...
        \begin{prooftree}
                        ...
                        \AxiomC{$\forall a . a \rightarrow a \sqsubseteq \texttt{Int} \rightarrow \texttt{Int}$}
                    \LeftLabel{Inst}
                    \BinaryInfC{$\Gamma, \text{id} : \texttt{Int} \rightarrow \texttt{Int}$}
                        \AxiomC{$4 : \texttt{Int} \in \Gamma$}
                        \RightLabel{Var}
                    \UnaryInfC{$\Gamma \vdash 4 : \texttt{Int}$}
                \RightLabel{App}
