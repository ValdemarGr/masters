...
\begin{figure}[ht]
    \centering
        \begin{tikzpicture}
            ...
            \node[draw=none, below = of top] (tv) {$a$};
...
\autoref{eq:diffeq} defines the free variables for some $\sigma$ under the assumption of some $\Gamma$ such that the definition states ``all free type variables in $\sigma$, that are not monotypes in $\Gamma$''.
This rule is important, since the system may not arbitrarily generalize over monotypes.
\begin{remark}
  When implementing type inference, generalization builds upon the idea of avoiding generalization of monotypes.
\end{remark}
%\begin{exmp}
