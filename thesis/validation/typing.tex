...
Natural deduction is expressed by \textit{inference rules}, which consist of one or more premises and a conclusion, or an axiom.
...
\end{figure}
Furthermore, hypotheses are written $\Gamma \vdash p$ which state that under the assumption of $\Gamma$ then $p$.
...
A very simple type system that models how the typed lambda calculus can be proved, must introduce a rule for each of the lambda calculus term types Var, Let, App and Abs.
...
\end{figure}
\begin{exmp}
  Using the rules for the simply typed lambda calculus, it becomes possible to prove the types for programs.
...
Note that a polymorphic type cannot introduce further quantifications inside of it's monotype. 
This is an important observation, since this constrains the \textit{rank} of the expressible types in Hindley-Milner.
The rank of a type is determined by how many independent quantification occur.
For instance, the type $\forall c. (\forall a.a \rightarrow c) \rightarrow c$ is not expressible and is in rank-2, since moving the quantifier $\forall a$ further out would weaken the type.
%In contrast, the type $\forall c . c \rightarrow (\forall a.a \rightarrow c)$ is rank-1, since moving the quantifier $\forall a$ out such that the type becomes $\forall a. \forall c. c \rightarrow (a \rightarrow c)$ does not weaken the type.
\begin{lemma}
  Type systems that allow quantifications to occur anywhere, are undecidable.
\end{lemma}
...
A principal component of typing in Hindley-Milner is the \textit{environment}, the environment is the type-theory equivalent to the hypotheses.
...
Generalization of a monotype into a polytype should quantify a variable \textbf{if and only if} it occurs free in that monotype, e.g. they must not occur free in any type in $\Gamma$, such that the generalisable variables of a monotype are $\textit{free}(\tau) \backslash \textit{free}(\Gamma)$.
...
For instance, a value \textit{id} of type $\forall a.a$ would both be valid to use in the expression $\textit{id } 5$ and $\textit{id} + 5$, which clearly do different things.
\begin{remark}
  The type $\forall a.a$ is clearly an absurd type (the function to which is called absurd in Haskell), since it allows one to bypass the type-system.
\end{remark}
...
This property is enforced by the Damas-Milner Algorithm W inference rules, by always combining substitutions if more than one occurs.
\end{remark}
...
As stated earlier, generalization is a matter of finding all type variables that do not occur free in $\Gamma$.
Type variables that do not occur free, do not necessarily imply bound, since they may have been introduced in the type that is to be generalized.
As such, a type variable may be quantified \textbf{if and only if} it is distinct from any other type variable.
Generalization of a type $\tau$, must be the polytype that becomes of the quantification of the type $\tau$ over $\textit{free}(\tau)\backslash\textit{free}(\Gamma) = \{\tau_1, \tau_2 \dots, \tau_n\}$ such that $\sigma \equiv \forall \tau_1 . \forall \tau_2 . \dots \forall \tau_n . \tau$.
...
\textit{inst} is an important component for allowing polymorphic types to remain polymorphic, since no bound type variable may be substituted.
\begin{exmp}
...
\end{exmp}
\begin{remark}
  An infinite function introduces a new way of inhabiting the type $\forall a.a$, by an expression such as $\texttt{let } x = x \texttt{ in } \dots$.
  The type $\forall a.a$ may seem invalid at first glance, but for a function that never ends it is the natural type.
\end{remark}
...
If algebraic data types were implemented as in \autoref{sec:adt} the type of an algebraic data type like Boolean in \autoref{lst:adtdecision} would be \texttt{a $\rightarrow$ a $\rightarrow$ a} since \texttt{BFalse} and \texttt{BTrue} must have a handler each.
...
%\end{minipage}
    ...
    The innermost \textit{dup} invocation will have its signature unified to $x \rightarrow (x \rightarrow x \rightarrow \tau) \rightarrow \tau$, if $a$ has type $x$ and $f$ has type $x \rightarrow x \rightarrow \tau$ for some unknown $\tau$ by the App rule in \autoref{fig:dmrules}.
    ...
    Naively one might judge \autoref{eq:ppe} to run in $2^n$ but an important observation for why \autoref{eq:ppe} does not induce exponential blowup is the uniqueness of the type variables.
...
\begin{lstlisting}[language=ML,caption={A function that requires a rank 2 type},label={lst:rankn},mathescape=true]
fun f makeNum a c =
    (makeNum a) + (makeNum c);
\end{lstlisting}
...
\begin{remark}
  To combat this, some languages let the programmer annotate the types of functions. 
\end{remark}
...
Let two matrices $m_1$ with the dimensions $i$ and $j$ encoded into $m_1$'s type, and $m_2$ with the dimensions $j$ and $j$ encoded into $m_2$'s type, be multiplied, then the resulting matrix $m_3$ has the dimensions $i$ and $k$, all of which can be encoded into the type system.
