...
\begin{figure}[ht]
    \begin{mdframed}[style=bigbox]
        ...
        \begin{prooftree}
                    ...
                    \LeftLabel{Abs}
                \UnaryInfC{$\Gamma \vdash (\lambda x . x) : \forall a . a \rightarrow a$}
                    \LeftLabel{Gen}
                    \AxiomC{$a \notin \textit{free}(\Gamma)$}
                \BinaryInfC{$\Gamma, x: \forall a. a \vdash x : \forall a . a$}
...
\begin{figure}[ht]
    \begin{mdframed}[style=bigbox]
        \begin{subfigure}[b]{1\textwidth}
        \begin{prooftree}
                            \AxiomC{$\text{id} : a \rightarrow a \in \Gamma$}
                            \LeftLabel{Var}
                        \UnaryInfC{$\Gamma \vdash \text{id} : a \rightarrow a$}
                        \AxiomC{$\forall a . a \rightarrow a \sqsubseteq \texttt{Int} \rightarrow \texttt{Int}$}
