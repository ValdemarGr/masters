...
For any function \texttt{f} which duplicates expressions and any state \texttt{$\forall$e.$\forall$x.\{x $\mapsto$ ($\dots$, e), f $\mapsto$ ($\dots$, ($\lambda$y.$\lambda$z.y+z))\}f x x} the only applicable rule is App.
Using the App rule two times yields a new state \texttt{$\forall$e.$\forall$x.\{x $\mapsto$ ($\dots$, e), f $\mapsto$ $\dots$, $\gamma_1$ $\mapsto$ ($\dots$, x), y $\mapsto$ ($\dots$, $\gamma_1$), $\gamma_2$ $\mapsto$ ($\dots$, x), z $\mapsto$ ($\dots$, $\gamma_2$)\}}.
...
\begin{figure}
  ...
  \label{fig:expdep}
\end{figure}
We have now arrived at the semantics (except lazy substitutions) described in~\cite{launchbury1993natural}.
