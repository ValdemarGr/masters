...
\begin{exmp}
  ...
  Let a program \texttt{(($\lambda$f.$\lambda$x.f x) ($\lambda$x.x)) 0} be subject to the rules in \autoref{fig:scbn}.
  \begin{figure}[ht]
    \begin{mdframed}
      \begin{prooftree}
        \AxiomC{\texttt{($\lambda$f.$\lambda$x.f x) ($\lambda$x.x)}}
        \AxiomC{}
        \RightLabel{Var}
...
Instead of performing $\alpha$-conversions one can leverage the ideas proposed in~\cite{launchbury1993natural} to implement laziness, lazy substitution, solve variable ambiguity and perform efficient garbage collection.
...
\begin{figure}[ht]
    \begin{mdframed}[style=style1]
        \vspace*{0.4cm}
          ...
          \end{prooftree}   
    \end{mdframed}
    \caption{An application rule which implements \autoref{eq:subsemfixed} and paired substitutions. (the entire set of rules can be found in \autoref{fig:subrenam})}
    \label{fig:subsapp}
\end{figure}
\begin{remark}
    ...
    \begin{mdframed}[style=bigbox]
        ...
        \begin{subfigure}[b]{1\textwidth}
          \vspace*{0.4cm}
              ...
              \begin{prooftree}
                \AxiomC{\texttt{$\Theta$, f $\rightarrow$ S, ($\lambda$x.e)}}
                \AxiomC{\texttt{\{$\gamma$ $\mapsto$ (S, z), x $\mapsto$ ($\emptyset$, $\gamma$)\} $\cup$ S, e $\rightarrow$ $\Sigma$, y} \,\,\,\, $\gamma$ = \texttt{fresh}}
                  \RightLabel{App}
                  \BinaryInfC{\texttt{$\Theta$, f z $\rightarrow$ $\Sigma$, y}}
              \end{prooftree}   
...
By using the App rule, one can introduce a variable by two different names.
A transitive reference is created to the originating expression, that is, two names which point to the same reference.
...
Using the App rule two times yields a new state \texttt{\{x $\mapsto$ ($\dots$, e), f $\mapsto$ $\dots$, $\gamma_1$ $\mapsto$ ($\dots$, x), y $\mapsto$ ($\dots$, $\gamma_1$), $\gamma_2$ $\mapsto$ ($\dots$, x), z $\mapsto$ ($\dots$, $\gamma_2$)\}}.
...
For instance if \texttt{y} was forced but \texttt{z} was not, then \texttt{z} would have to evaluate the reference to $\gamma_2$ which points to \texttt{x} which has been evaluated and added to the substitution mapping by \autoref{fig:rules:freshenv:var} since \texttt{x} is a (transitive) parent of \texttt{y}.
\begin{figure}
  \centering
          ...
          \node[draw=black] (e) {\texttt{e}};
            ...
            \path[->] (x) edge node[left] {} (e);
              ...
              \path[->] (gamma1) edge node[left] {} (x);
                ...
                \node[draw=black, below = of gamma1] (y) {\texttt{y}};
                \path[->] (y) edge node[left] {} (gamma1);

              \node[draw=black, below right = of x] (gamma2) {$\gamma_2$};
              \path[->] (gamma2) edge node[left] {} (x);

                \node[draw=black, below = of gamma2] (z) {\texttt{z}};
                \path[->] (z) edge node[left] {} (gamma2);
...
Garbage collection in purely functional languages is different from imperative languages like C, since manual memory management is often abstracted away. 
...
\begin{remark}
  Continually passing style programs clearly need some type of garbage collection.
...
\end{remark}
\noindent\cite{launchbury1993natural}~presents an elegant (and abstract) rule for garbage collection (\autoref{fig:launchgc}, and a version with lazy substitutions \autoref{fig:lazysub:gc}).
...
The lazy substitution semantics might appear foreign to the rules of garbage collection but can indeed be benefitial in regard to the function \texttt{R}.
The implementation of the function \texttt{R} is very open for interpretation.
A garbage collection algoritm which collects every ``lost reference'' which adheres to \texttt{R} in \autoref{fig:launchgc}, could be implemented by the use of \texttt{Free} or \texttt{Bound}.
A garbage collection algoritm for \autoref{fig:lazysub:gc} must also inspect the bound substitution mappings as in \autoref{eq:Rgc}.
\begin{align}
  \texttt{R($\Theta$, ($\lambda$x.e))}&\texttt{ = R($\Theta$ $\backslash$ \{x\}, e)}& \label{eq:Rgc}\\
   \texttt{R($\Theta$, (f e))}&\texttt{ = R($\Theta$, f) $\cup$ R($\Theta$, e)}& \tag*{} \\
   \texttt{R($\Theta$, x)}&\texttt{ = \{x $\mapsto$ (e, R(S, e))\} $\cup$ R(S, e)}&, \texttt{x $\mapsto$ (e, S) $\in$ $\Theta$} \tag*{} \\
   \texttt{R($\Theta$, x)}&\texttt{ = \{\}}&, \texttt{x $\mapsto$ (e, S) $\notin$ $\Theta$} \tag*{}
\end{align}

Garbage collection algorithms often run in worst case $O(n)$, in regard to the number of variables allocated by the App rule.
The algorithm proposed in \autoref{eq:Rgc} also has similar guarantees, if \texttt{R} is implemented efficiently.
The garbage collection algorithm is entirely dependent on how an implementation of \texttt{R} behaves.
...
Fortunately, garbage collection can be paid for ``up front''.
The compilation can  \autoref{fig:expdep}
