...
\begin{figure}
\begin{lstlisting}[language=ML,caption={Program that implements two functions that map a \texttt{List a} to a \texttt{List b}},label={lst:listmapboth},mathescape=true]
type List a = 
    | Nil
    | Cons a (List a)
;
...
To really understand what happens in a lambda calculus interpreter, we must understand what happens in \autoref{lst:listmapboth}.
\autoref{lst:listmapboth} implements two variants of a \texttt{map} function which maps a list of type \texttt{List a} to a list \texttt{List b}.
...
\begin{remark}
    ...
    For instance the expression \texttt{let f = ($\lambda$g.$\lambda$x.g x) $\dots$ f g' 0} is eventually rewritten to \texttt{g' 0}.
    If for instance the expression awaited a result like in \texttt{let f = ($\lambda$g.$\lambda$x.x + (g x)) $\dots$ f g' 0}, then it would be rewritten to \texttt{x + (g' 0)}, which could grow the program throughout interpretation.
    It should become clear that reduction strategies always implies tail call optimization.
\end{remark}
The first flavor of \texttt{map}; \texttt{mapTail}, implements \texttt{map} such that the program expression tree does not grow throughout program interpretation, under a call by value environment.
\\
\begin{minipage}{\textwidth}
\begin{lemma}
    Evaluating \texttt{mapTail} on a list of arbitary size, adds at most $O(1)$ terms, at any one time throughout program rewriting.
    Equivalently, at any one point of the evaluation of \texttt{mapTail} the size in terms of lambda terms of \texttt{mapTail} is at most $O(n)$.
\end{lemma}
\begin{proof}
    ...
    \begin{align}
        ...
        =&\texttt{ mapImpl (Cons 2 (Cons 3 $\dots$ (Cons n Nil))) ($\lambda$x.x) (Cons 1)}  \tag*{} \\
        =&\texttt{ r (b Nil) ($\lambda$x.$\lambda$xs.mapTail xs f (b (Cons (f x))))}  \tag*{} \\
        =&\texttt{ mapTail xs f (($\lambda$xs.Cons 1 xs) (Cons 2))}  \tag*{} \\
        =&\texttt{ mapTail xs f (($\lambda$xs.Cons 1 xs) (Cons 2))}  \tag*{} \\
        =&\texttt{ mapTail xs f (Cons 1 (Cons 2))}  \tag*{} \\
        =&\texttt{ mapTail (Cons 3 (Cons 4 $\dots$ (Cons n Nil))) ($\lambda$x.x) (Cons 1 (Cons 2))}  \eqtag{3$\dots$n} \\
        =&\texttt{ r (b Nil) ($\lambda$x.$\lambda$xs.mapTail xs f (b (Cons (f x))))}  \tag*{} \\
        =&\texttt{ mapTail xs f (($\lambda$xs.Cons 1 (Cons 2 xs)) (Cons 3))}  \tag*{} \\
        =&\texttt{ mapTail (Cons 4 (Cons 5 $\dots$ (Cons n Nil))) ($\lambda$x.x) (Cons 1 (Cons 2 (Cons 3)))}  \eqtag{4$\dots$n} \\
        =&\dots\tag*{}\\
        =&\texttt{ mapTail (Cons k (Cons (k + 1) $\dots$ (Cons n Nil)))} \tag*{} \\
        &\texttt{ ($\lambda$x.x) (Cons 1 ($\dots$ (Cons (k - 1))))}  \tag*{} 
    \end{align}
    For any one $k$, the combined size of the two lists must be $1, \dots k - 1, k, \dots n$.
\end{proof}
\end{minipage}
