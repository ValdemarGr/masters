...
A function which garbage collects every ``lost reference'' which adheres to \texttt{R} in \autoref{fig:launchgc} could be implemented by the use of \texttt{Free}.
\begin{align}
  & \texttt{R($\Theta$, ($\lambda$x.e)) &= R($\Theta$ $\backslash$ \{x\}, e)}&\\
  & \texttt{R($\Theta$, (f, e)) &= R($\Theta$, f) $\cup$ R($\Theta$, e)}& \tag*{} \\
  & \texttt{R($\Theta$, x) &= \{x $\mapsto$ (e, S)\} $\cup$ R(S, e)}& \texttt{x $\mapsto$ (e, S) $\in$ $\Theta$} \tag*{} \\
  & \texttt{R($\Theta$, x) &= \{\}}& \texttt{x $\mapsto$ (e, S) $\notin$ $\Theta$} \tag*{}
\end{align}
