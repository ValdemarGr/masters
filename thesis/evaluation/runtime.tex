...
\begin{figure}[ht]
    ...
    \label{fig:rules:env}
\end{figure}
The $\cup$ operator merges two substitution mappings, choosing the rightmost mapping on duplicates, such that \texttt{\{x $\mapsto$ y, z $\mapsto$ h\} $\cup$ \{x $\mapsto$ m\} $\rightarrow$ \{x $\mapsto$ m, z $\mapsto$ h\}}.
The semantics of $\cup$ satisfy \autoref{eq:subsemfixed}.
...
The subtle difference is that the binding of an expression involves binding the expression itself and its accompanying environment.
\begin{remark}
  ...
  \texttt{e} is an abstract representation of something, while the environment \texttt{S} is the necessary values to instantiate \texttt{e}.
  Clearly curried functions can also be seen this way, but it remains an interesting observation.
\end{remark}
