...
Furthermore if no precaution are taken in $\lambda\texttt{x.g x}$ then $\lambda\texttt{g.}$ on line 4 will leak into the substitution mapping. 
...
The problem of solving closures becomes significantly easier when either applying De Bruijn indices (\autoref{todo}) or persistent data structures (\autoref{todo}) to the problem.
Applying De Bruijn indices has additional advantages that have an impact on the method of evaluating, to keep the context in the domain of named variables persistent data structures will be the focus.
...
\begin{align}
    ...
    = \,\,\,& O \left( n^3 \right)\tag*{}
\end{align}
Since persistence is guaranteed all pointers are simply pointers to previous states of the substitution mapping.
...
\begin{align}
    ...
    &\textit{force}(\texttt{f}, \{ \texttt{f} \mapsto \lambda \texttt{x.x}, \texttt{z} \mapsto \texttt{f 0} \}) = \{ \texttt{f} \mapsto \lambda \texttt{x.x}, \texttt{z} \mapsto 0 \} \label{eq:eval:need} \eqtag{Call by need}
\end{align}
...
\begin{figure}[ht]
    \begin{mdframed}[style=style1]
        ...
        \begin{subfigure}[b]{0.59\textwidth}
            \begin{prooftree}
                \AxiomC{$S \cdot \{ x \mapsto e_2 \},e'_1$}
                \AxiomC{$Se_1 \equiv \lambda x . e'_1$}
                \RightLabel{App}
                \BinaryInfC{$S,e_1 e_2$}
            \end{prooftree}   
