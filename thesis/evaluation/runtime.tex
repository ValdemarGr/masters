...
Imperative languages are also often evaluated under call by value which further simplifies implementation details.
...
//List a $\rightarrow$ (a $\rightarrow$ b) $\rightarrow$ (List b $\rightarrow$ List b) $\rightarrow$ List b
    ...
    match r
        | Nil -> Nil;
        | Cons x xs -> mapTail xs f (Cons (f x));
    ;
fun mapGrowing f l = 
    match l
        | Nil -> Nil;
    ...
    ;
\end{lstlisting}
In pure functional programming languages, techniques such as \textit{tail call optimization} exist
