...
Translating the lambda calculus into the SKI calculus does indeed increase the size significantly but does not warrant a write off entirely.
...
It should become clear that an $\alpha$-conversion algorithm must also follow the reduction order or else one can force terrible runtime by creating purposeless terms which are never executed but are converted.
