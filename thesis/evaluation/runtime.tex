...
A heap contains mappings from variables to expressions, much like the substitution mapping.
Heaps are a requirement for call by need semantics.
A simple modification to the rules in \autoref{fig:rules:env}, implies that the system must bring a heap along. 
\begin{figure}[ht]
    \begin{mdframed}[style=bigbox]
        \vspace*{0.49cm}
...
More precisely, when evaluating \texttt{f} in the App rule in \autoref{fig:rules:env:app}, the expression \texttt{z} should not be released since it must be present for the right branch (\texttt{\{x $\mapsto$ z\}e}).
...
\begin{figure}[ht]
  \begin{mdframed}[style=style1]
    ...
    \vspace*{0.19cm}
          \begin{prooftree}
            \AxiomC{\texttt{$\Theta$, f $\rightarrow_{\texttt{(N $\cup$ \{z\})}}$ S, ($\lambda$x.e)}}
            \AxiomC{\texttt{S, \{x $\mapsto$ z\}e $\rightarrow_{\texttt{N}}$ $\Sigma$, y}}
              \RightLabel{App}
              \BinaryInfC{\texttt{$\Theta$, f z $\rightarrow_{\texttt{N}}$ $\Sigma$, y}}
          \end{prooftree}   
    \end{subfigure}
    \begin{subfigure}[b]{1\textwidth}
    \vspace*{0.49cm}
...
\begin{figure}[ht]
  \begin{mdframed}
    \begin{prooftree}
            \AxiomC{\texttt{$\Theta$, e $\rightarrow_{\texttt{N}}$ S, y}}
            \AxiomC{\texttt{x $\notin$ R($\Theta$, N, e)}}
              \RightLabel{GC}
              \BinaryInfC{\texttt{$\Theta$ $\cup$ \{x $\mapsto$ z\}, e $\rightarrow_{\texttt{N}}$ S, y}}
    \end{prooftree}   
  \end{mdframed}
...
\begin{figure}[ht]
  \begin{mdframed}
    \begin{prooftree}
            \AxiomC{\texttt{$\Sigma$, e $\rightarrow_{\texttt{N}}$ S, y}}
            \AxiomC{\texttt{$\Sigma$ = Prune($\Theta$, N)}}
              \RightLabel{GC}
              \BinaryInfC{\texttt{$\Theta$ $\cup$ \{x $\mapsto$ z\}, e $\rightarrow_{\texttt{N}}$ S, y}}
    \end{prooftree}   
  \end{mdframed}
...
\begin{align}
  &\texttt{Prune($\Theta$, \{\}) = \{\}} \label{eq:prune}\\
  &\texttt{Prune($\Theta$, \{e\} $\cup$ N) = \{x $\mapsto$ y | x $\in$ free(e), x $\mapsto$ y $\in$ $\Theta$\} $\cup$ Prune($\Theta$, N)} \tag*{}
\end{align}
...
One can implement the machine very closely to what the semantics describe, as an initial implementation.
