...
\]
Adding a rule which allows only the innermost substitution to be captured yields the two following rules (\autoref{eq:subsemfixed}) instead of \autoref{eq:subsemlaz2}.
\begin{align}
  &S (\lambda x.e) = (\lambda x.Se) \label{eq:subsemfixed} & (x \mapsto y) \notin S \tag*{}\\
  &S (\lambda x.e) = (\lambda x.(S \backslash \{x \mapsto y\})e) & (x \mapsto y) \in S \tag*{}%\label{eq:subsemlaz2}
\end{align}
