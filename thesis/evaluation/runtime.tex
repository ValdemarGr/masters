...
f 10;
\end{lstlisting}
Evaluating \autoref{lst:problemprog} using any reduction method, yields an interesting case of variable ambiguity.
...
Now let \texttt{E} be \texttt{($\lambda$x.let g = f in g) 10} such that evaluation yields \autoref{eq:red}.
\begin{align}
  &\texttt{S = \{x $\mapsto$ \{$\dots$\},10, f $\mapsto$ \{x $\mapsto$ 0\}($\lambda$y.x + y)\}} \tag*{}\\
...
\end{align}
\autoref{eq:red} is a common closure and partial application problem in disquise.
...
By renaming all variables to a unique name when they are introduced, the evaluation model can be simplified significantly and allows \textit{any} of the afforementioned application rules.
The operation of renaming is called $\alpha$-conversions.
\begin{remark}
  Renaming might render the work of deriving lazy substituions meaningless, but there remains silver lining.
